...
function grad = ptv_partial_conv(gG, ksz, grid_spacing, K_ord, KT)
    ...
    for i = 1 : Nimgs
        switch K_ord
            case 1
                if Nd == 2
                    [gr1, gr2] = mex_2d_linear_partial_conv_double(gG(:,:, 1, 1, i, 1), gG(:,:, 1, 1, i, 2), ksz, grid_spacing(1:2));
                else
...
%     Nd = size(Knots_pix, 4);
    ...
    for i = 1 : Nimgs
        ...
        for j = 1 : Nch
            if ~manual_mode
                if Nd == 2
                    [vt, g1, g2] = mex_linear_disp_and_warp_and_grad_2d(squeeze(Knots_pix(:,:,1, :, i)), ...
...
%                         [g1,g2] = warp_gradient(squeeze(volmov(:,:,:, j, i)), squeeze(Tpix), pix_resolution(1:Nd), interp_type, out_type);
                        [vt, G] = mex_imdeform2_and_grad(squeeze(volmov(:,:,:, j, i)), squeeze(Tpix), interp_type);
