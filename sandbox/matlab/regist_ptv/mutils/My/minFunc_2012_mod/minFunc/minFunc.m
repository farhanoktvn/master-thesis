function [x,f,exitflag,output, custom_outp] = minFunc(funObj,x0,options,varargin)
...
custom_outp = getoptions(options, 'custom_outp', []);
        ...
        case PCG % Preconditioned Non-Linear Conjugate Gradient
			...
			if isempty(precFunc)
				...
				if i == 1
					...
					if useMex
						s = lbfgsProdC(g,S,Y,YS,int64(lbfgs_start),int64(lbfgs_end),Hdiag);
					else
        ...
        case LBFGS % L-BFGS
			...
			if Damped
				...
				if i == 1
					...
					lbfgs_end = 0;
					Hdiag = 1;
                    if ~isempty(custom_outp)
                        if true||isfield(custom_outp, 'S')
                            S = custom_outp.S;
                            Y = custom_outp.Y;
                            YS = custom_outp.YS;
                            lbfgs_start = custom_outp.lbfgs_start;
                            lbfgs_end = custom_outp.lbfgs_end;
                            Hdiag = custom_outp.Hdiag;
                            if useMex
                                d = lbfgsProdC(g,S,Y,YS,int64(lbfgs_start),int64(lbfgs_end),Hdiag);
%                                 d2 = lbfgsProd(g,S,Y,YS,lbfgs_start,lbfgs_end,Hdiag);
%                                 fprintf('Prod %e\n', max(abs(d(:)-d2(:))));
%                                 pause
                            else
					...
					if true
                        ...
                        custom_outp.Y = Y;
                        custom_outp.YS = YS;
                        custom_outp.lbfgs_start = lbfgs_start;
                        custom_outp.lbfgs_end = lbfgs_end;
                        custom_outp.Hdiag = Hdiag;
                    end
                    if debug && skipped
						fprintf('Skipped L-BFGS updated\n');
					end
			...
			if cgSolve == 1
				if isempty(precFunc) % Apply L-BFGS preconditioner
					if i == 1
						...
						precondArgs = {S,Y,YS,int64(lbfgs_start),int64(lbfgs_end),Hdiag};
					end
					g_old = g;
				else
...
%         return
    ...
    if i == 1
        ...
        if LS_init == 5 || LS_init == 6 || LS_init == 7 || LS_init == 8
            tmp = sort(abs(g));
            tmp = tmp(ceil(end*0.9));
            t = 5 ./ tmp;
            t = 1 ./ tmp;
%             A_step_length = getoptions(options, 'A_step_length', 1);
%             [t, max(abs(d)), max(abs(g))]
%             t = t * A_step_length;
%            t = min(1,1/sum(abs(g)));
        end
    elseif i == 2
...
%             t = t * 1.2;
    ...
    if LS_type == 0 % Use Armijo Bactracking
        ...
        cap_values = []; %getoptions(options, 'cap_values', []);
        if computeHessian
            [t,x,f,g,LSfunEvals,H] = ArmijoBacktrack(cap_values,x,t,d,f,fr,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,LS_saveHessianComp,funObj,varargin{:});
        else
            [t,x,f,g,LSfunEvals] = ArmijoBacktrack(cap_values,x,t,d,f,fr,g,gtd,c1,LS_interp,LS_multi,progTol,debug,doPlot,1,funObj,varargin{:});
        end
%         t
    ...
    elseif LS_type == 1 % Find Point satisfying Wolfe conditions
        ...
        if computeHessian
            ...
            if i == 1 && ~isempty(custom_outp) && isfield(custom_outp, 't')
%                 t = custom_outp.t;
            end
...
%     end
    ...
    if abs(f-f_old) < progTol || f == f_old
        exitflag=2;
