...
reg_label_img_folder = Path(cwd, 'reg-label-imgs')
...
class RunLoader:
    ...
    def __init__(
            ...
            label_img_save_path=None
        ):
        self.sample_id = sample_id
        self.run_group = run_group
        self.run_id = run_id
        self.label_save_path = label_save_path
        self.label_img_save_path = label_img_save_path
    ...
    def register_label(self):
        ...
        try:
            ...
            self.label_img = cv2.warpPerspective(label_img, self.M, (w, h))
            self.label_mask = cv2.warpPerspective(label_mask, self.M, (w, h))
    ...
    def save_mi(self):
        if self.label_img_save_path is None:
            return

        # Save label image
        label_img_path = Path(self.label_img_save_path, self.sample_id, 'label_img', f'{self.run_id}.jpg')
        label_img_path.parent.mkdir(parents=True, exist_ok=True)
        cv2.imwrite(str(label_img_path), self.label_img)

        label_json_path = Path(self.label_img_save_path, self.sample_id, 'label_json', f'{self.run_id}.json')
        label_json_path.parent.mkdir(parents=True, exist_ok=True)
        with open(label_json_path, 'w', encoding='utf-8') as f:
            json.dump({'M': self.M}, f, ensure_ascii=False, indent=4, cls=NumpyEncoder)

    
    def save_label(self):
        if self.label_save_path is None:
            return
        ...
        for run_group in tqdm.tqdm(run_groups, desc='Run Groups', position=1, leave=False):
            for run_id in tqdm.tqdm(run_group.get('runs'), desc='Runs', position=2, leave=False):
                run_data = RunLoader(
                    ...
                    label_save_path=reg_label_folder,
                    label_img_save_path=reg_label_img_folder
                )
