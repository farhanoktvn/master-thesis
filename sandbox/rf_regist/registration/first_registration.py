...
class Align:
    def __init__(self, source, target, mode=DEFAULT_MODE, K=3, threshold=255):
        self.source = source
        self.target = target
        self.mode = mode
        self.K = K
    ...
    def align_image(self):
        img_source = self.source.copy()
        img_target = self.target.copy()

        img_source_cp = img_source.copy()
        img_source_cp = cv2.equalizeHist(img_source_cp)
        img_target_cp = img_target.copy()
        img_target_cp = cv2.equalizeHist(img_target_cp)

        kp_s, desc_s = self.extract_SIFT(img_source_cp)
        kp_t, desc_t = self.extract_SIFT(img_target_cp)
