...
def getWHTensor(feat):
    W = (
        torch.arange(0, feat.size(2), device=feat.device)
        .view(-1, 1)
        .expand(feat.size(2), feat.size(3))
        .contiguous()
        .view(-1)
        .float()
        + 0.5
    ...
    H = (
        torch.arange(0, feat.size(3), device=feat.device)
        .view(1, -1)
        .expand(feat.size(2), feat.size(3))
        .contiguous()
        .view(-1)
...
def mutualMatching(featA, featB):
    ...
    keepMaxDim0 = torch.zeros((featA.size(1), featB.size(1)), device=featA.device).scatter_(
        0, maxDim0Index, maxDim0
    )
    keepMaxDim1 = torch.zeros((featA.size(1), featB.size(1)), device=featA.device).scatter_(
        1, maxDim1Index, maxDim1
    )
...
def Affine(X, Y):
    ...
    H21 = H21.T
    H21 = np.array(
        [[H21[0, 0], H21[0, 1], H21[0, 2]], [H21[1, 0], H21[1, 1], H21[1, 2]], [0, 0, 1]]
    )
...
def Hough(X, Y):
    ...
    H21 = np.array([[H21x[0], 0, H21x[1]], [0, H21y[0], H21y[1]], [0, 0, 1]])
    return H21
...
def Homography(X, Y):
    ...
    for i in range(4):
        ...
        A[:, 2 * i] = np.stack(
            [np.zeros(N), np.zeros(N), np.zeros(N), -u, -v, -np.ones(N), v_ * u, v_ * v, v_], axis=1
        )
        A[:, 2 * i + 1] = np.stack(
            [u, v, np.ones(N), np.zeros(N), np.zeros(N), np.zeros(N), -u_ * u, -u_ * v, -u_], axis=1
        )
...
def Translation(X, Y):
    tx = X[0, 0] - Y[0, 0]
    ty = X[0, 1] - Y[0, 1]
    H21 = np.array([[1, 0, tx], [0, 1, ty], [0, 0, 1]])
    return H21
...
def RANSAC(nbIter, match1, match2, tolerance, nbPoint, Transform):
    ...
    for i in range(nbLoop):
        H21, nbInlier = ScoreRANSAC(
            match1, match2, tolerance, unique_samples.narrow(0, i * nbMaxIter, nbMaxIter), Transform
        )
    ...
    if len(unique_samples) - nbLoop * nbMaxIter > 0:
        H21, nbInlier = ScoreRANSAC(
            ...
            match2,
            tolerance,
            unique_samples.narrow(0, nbLoop * nbMaxIter, len(unique_samples) - nbLoop * nbMaxIter),
            Transform,
        )
    ...
    isInlier = error < tolerance
    return (
        bestParams.cpu().numpy(),
        bestInlier.cpu().numpy(),
        isInlier.cpu().numpy(),
        match2[isInlier].cpu().numpy(),
    )
...
def SaliencyCoef(feat):
    ...
    tmpCoef = torch.cat(
        [
            torch.sum(
                tmpFeat.narrow(start=2, dim=2, length=w).narrow(start=1, dim=3, length=h) * feat,
                dim=1,
                keepdim=True,
            ),
            torch.sum(
                tmpFeat.narrow(start=0, dim=2, length=w).narrow(start=1, dim=3, length=h) * feat,
                dim=1,
                keepdim=True,
            ),
            torch.sum(
                tmpFeat.narrow(start=1, dim=2, length=w).narrow(start=0, dim=3, length=h) * feat,
                dim=1,
                keepdim=True,
            ),
            torch.sum(
                tmpFeat.narrow(start=1, dim=2, length=w).narrow(start=2, dim=3, length=h) * feat,
                dim=1,
                keepdim=True,
            ),
        ],
