...
class CoarseAlign:
    def __init__(
        ...
        if imageNet:
            ...
            featPth = "/Users/farhanoktavian/imperial/thesis/sandbox/final_regist/weights/resnet50_moco.pth"
        ...
        if transform == "Affine":
            self.Transform = utils.Affine
            self.nbPoint = 3

        else:
            self.Transform = utils.Homography
            self.nbPoint = 4
        ...
        if nbScale == 1:
            ...
            self.scaleList = (
                np.linspace(scaleR, 1, nbScale // 2 + 1).tolist()
                + np.linspace(1, 1 / scaleR, nbScale // 2 + 1).tolist()[1:]
            )
    ...
    def ResizeMaxSize(self, I, minSize):
        ...
        new_w, new_h = (
            new_w // self.strideNet * self.strideNet,
            new_h // self.strideNet * self.strideNet,
        )
    ...
    def setTarget(self, It_org):
        with torch.no_grad():
            ...
            self.Wt, self.Ht = utils.getWHTensor(self.featt)
    ...
    def getCoarse(self, Mt):
        ...
        with torch.no_grad():
            ...
            MtTensor = F.interpolate(
                input=MtExtend, size=(self.featt.size()[2], self.featt.size()[3]), mode="bilinear"
            )
            ...
            index1, index2 = utils.mutualMatching(self.featsMultiScale, featt)
            ...
            match1 = torch.cat(
                (H1MutualMatch.unsqueeze(1), W1MutualMatch.unsqueeze(1), ones.unsqueeze(1)), dim=1
            )
            match2 = torch.cat(
                (H2MutualMatch.unsqueeze(1), W2MutualMatch.unsqueeze(1), ones.unsqueeze(1)), dim=1
            )

            if len(match1) < self.nbPoint:
                return None, []
            bestParam, _, indexInlier, _ = utils.RANSAC(
                self.nbIter, match1, match2, self.tolerance, self.nbPoint, self.Transform
            )
            ...
            if bestParam is None:
                ...
                InlierMask = np.zeros(
                    (self.featt.size()[2], self.featt.size()[3]), dtype=np.float32
                )
                InlierMask[
                    ((self.Wt[index2Inlier] / 2 + 0.5) * self.featt.size()[2])
                    .cpu()
                    .numpy()
                    .astype(np.int64),
                    ((self.Ht[index2Inlier] / 2 + 0.5) * self.featt.size()[3])
                    .cpu()
                    .numpy()
                    .astype(np.int64),
                ] = 1
